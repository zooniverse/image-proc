...
/**
 * Hash the crop params filename
 *
 */
image.prototype.cropHashFilename = function(w,h,x,y) {
  var img_url = this.url + w + h + x + y;
  this.filenameHashedCrop = crypto.createHash('md5').update(img_url).digest("hex");
  return this.path + this.filenameHashedCrop + '.' + this.extension;
}
